...
{
    public class PKTicketService : IPKTicketService
    {
        private readonly ApplicationDbContext _context;
        private readonly IPKRoleService _roleService;
        private readonly IPKProjectService _projectService;

        public PKTicketService(ApplicationDbContext context, IPKRoleService roleService, IPKProjectService projectService)
        {
            _context = context;
        ...
        public async Task<PKUser> GetTicketDeveloperAsync(int ticketId, int companyId)
        {
            PKUser developer = new();
        ...
        {
            PKUser pkUser = await _context.Users.FirstOrDefaultAsync(u => u.Id == userId);
            List<Ticket> tickets = new();

            try
            {
                if (await _roleService.IsUserInRoleAsync(pkUser, Roles.Admin.ToString()))
                {
                    tickets = (await _projectService.GetAllProjectsByCompany(companyId)).SelectMany(p => p.Tickets).ToList();
                }
                else if (await _roleService.IsUserInRoleAsync(pkUser, Roles.Developer.ToString()))
                {
                    tickets = (await _projectService.GetAllProjectsByCompany(companyId))
                                                    .SelectMany(p => p.Tickets)
                                                    .Where(t => t.DeveloperUserId == userId).ToList();
                }
                else if (await _roleService.IsUserInRoleAsync(pkUser, Roles.Submitter.ToString()))
                {
                    tickets = (await _projectService.GetAllProjectsByCompany(companyId))
                                                    .SelectMany(p => p.Tickets)
                                                    .Where(t => t.OwnerUserId == userId).ToList();
                }
                else if (await _roleService.IsUserInRoleAsync(pkUser, Roles.ProjectManager.ToString()))
                {
