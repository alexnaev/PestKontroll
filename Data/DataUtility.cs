...
namespace PestKontroll.Data
{
    ...
    public static class DataUtility
    {
        ...
        public static async Task ManageDataAsync(IHost host)
        {
            ...
            var userManagerSvc = svcProvider.GetRequiredService<UserManager<PKUser>>();
        ...
        public static async Task SeedDefaultProjectPriorityAsync(ApplicationDbContext context)
        {
            try
            {
                IList<Models.ProjectPriority> projectPriorities = new List<ProjectPriority>() {
                                                    new ProjectPriority() { Name = PKProjectPriority.Low.ToString() },
                                                    new ProjectPriority() { Name = PKProjectPriority.Medium.ToString() },
                                                    new ProjectPriority() { Name = PKProjectPriority.High.ToString() },
                                                    new ProjectPriority() { Name = PKProjectPriority.Urgent.ToString() },
                };
        ...
        public static async Task SeedDefautProjectsAsync(ApplicationDbContext context)
        {
            ...
            int priorityLow = context.ProjectPriorities.FirstOrDefault(p => p.Name == PKProjectPriority.Low.ToString()).Id;
            int priorityMedium = context.ProjectPriorities.FirstOrDefault(p => p.Name == PKProjectPriority.Medium.ToString()).Id;
            int priorityHigh = context.ProjectPriorities.FirstOrDefault(p => p.Name == PKProjectPriority.High.ToString()).Id;
            int priorityUrgent = context.ProjectPriorities.FirstOrDefault(p => p.Name == PKProjectPriority.Urgent.ToString()).Id;
        ...
        public static async Task SeedDefaultUsersAsync(UserManager<PKUser> userManager)
        {
            //Seed Default Admin User
            var defaultUser = new PKUser
            {
        ...
        public static async Task SeedDemoUsersAsync(UserManager<PKUser> userManager)
        {
        ...
        public static async Task SeedDefaultTicketTypeAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketType> ticketTypes = new List<TicketType>() {
                     new TicketType() { Name = PKTicketType.NewDevelopment.ToString() },      // Ticket involves development of a new, uncoded solution 
                     new TicketType() { Name = PKTicketType.WorkTask.ToString() },            // Ticket involves development of the specific ticket description 
                     new TicketType() { Name = PKTicketType.Defect.ToString()},               // Ticket involves unexpected development/maintenance on a previously designed feature/functionality
                     new TicketType() { Name = PKTicketType.ChangeRequest.ToString() },       // Ticket involves modification development of a previously designed feature/functionality
                     new TicketType() { Name = PKTicketType.Enhancement.ToString() },         // Ticket involves additional development on a previously designed feature or new functionality
                     new TicketType() { Name = PKTicketType.GeneralTask.ToString() }          // Ticket involves no software development but may involve tasks such as configuations, or hardware setup
                };
        ...
        public static async Task SeedDefaultTicketStatusAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketStatus> ticketStatuses = new List<TicketStatus>() {
                    new TicketStatus() { Name = PKTicketStatus.New.ToString() },                 // Newly Created ticket having never been assigned
                    new TicketStatus() { Name = PKTicketStatus.Development.ToString() },         // Ticket is assigned and currently being worked 
                    new TicketStatus() { Name = PKTicketStatus.Testing.ToString()  },            // Ticket is assigned and is currently being tested
                    new TicketStatus() { Name = PKTicketStatus.Resolved.ToString()  },           // Ticket remains assigned to the developer but work in now complete
                };
        ...
        public static async Task SeedDefaultTicketPriorityAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketPriority> ticketPriorities = new List<TicketPriority>() {
                                                    new TicketPriority() { Name = PKTicketPriority.Low.ToString()  },
                                                    new TicketPriority() { Name = PKTicketPriority.Medium.ToString() },
                                                    new TicketPriority() { Name = PKTicketPriority.High.ToString()},
                                                    new TicketPriority() { Name = PKTicketPriority.Urgent.ToString()},
                };
        ...
        public static async Task SeedDefautTicketsAsync(ApplicationDbContext context)
        {
            ...
            //Get ticket type Ids
            int typeNewDev = context.TicketTypes.FirstOrDefault(p => p.Name == PKTicketType.NewDevelopment.ToString()).Id;
            int typeWorkTask = context.TicketTypes.FirstOrDefault(p => p.Name == PKTicketType.WorkTask.ToString()).Id;
            int typeDefect = context.TicketTypes.FirstOrDefault(p => p.Name == PKTicketType.Defect.ToString()).Id;
            int typeEnhancement = context.TicketTypes.FirstOrDefault(p => p.Name == PKTicketType.Enhancement.ToString()).Id;
            int typeChangeRequest = context.TicketTypes.FirstOrDefault(p => p.Name == PKTicketType.ChangeRequest.ToString()).Id;

            //Get ticket priority Ids
            int priorityLow = context.TicketPriorities.FirstOrDefault(p => p.Name == PKTicketPriority.Low.ToString()).Id;
            int priorityMedium = context.TicketPriorities.FirstOrDefault(p => p.Name == PKTicketPriority.Medium.ToString()).Id;
            int priorityHigh = context.TicketPriorities.FirstOrDefault(p => p.Name == PKTicketPriority.High.ToString()).Id;
            int priorityUrgent = context.TicketPriorities.FirstOrDefault(p => p.Name == PKTicketPriority.Urgent.ToString()).Id;

            //Get ticket status Ids
            int statusNew = context.TicketStatuses.FirstOrDefault(p => p.Name == PKTicketStatus.New.ToString()).Id;
            int statusDev = context.TicketStatuses.FirstOrDefault(p => p.Name == PKTicketStatus.Development.ToString()).Id;
            int statusTest = context.TicketStatuses.FirstOrDefault(p => p.Name == PKTicketStatus.Testing.ToString()).Id;
            int statusResolved = context.TicketStatuses.FirstOrDefault(p => p.Name == PKTicketStatus.Resolved.ToString()).Id;
